...
class container;
  ...
  rand item_t items[11];
  // Generates items[-1]
  constraint less_c {foreach (items[i]) if (i !== 0) items[i-1].value < 110;}
  // Generates items[7]
  constraint greater_c {foreach (items[i]) if (i !== 6) items[i+1].value > 50;}
  ...
  constraint arr_index_not_wide_c2 {
    items.xor() with (item.index > 0 ? items[item.index-1].value : items[item.index].value) != 0;
  }
  function new();
