...
#define LINKDOT_VISIT_START() \
    ...
    void visit(AstRefDType* nodep) override {
        ...
        if (AstNode* cpackagep = nodep->classOrPackageOpp()) {
            ...
            if (AstDot* const dotp = VN_CAST(cpackagep, Dot)) {
                cpackagerefp = resolveNestedTypes(dotp);
                // Couldn't resolve nested types, errors reported earlier
                if (!cpackagerefp) return;
                // The parents are resolved and only named the reference below them, so
                // collapse the chain into that reference. Later passes (V3Param) then see
                // the same shape as a non-nested 'class#(param)::type' reference.
                dotp->replaceWith(cpackagerefp->unlinkFrBack());
                VL_DO_DANGLING(pushDeletep(dotp), dotp);
                cpackagep = cpackagerefp;
            }
