...
class WidthVisitor final : public VNVisitor {
    ...
    bool warnSelTriAndReplace(AstNodeExpr* nodep, AstConst* lsbConstp) {
        if (!lsbConstp->num().isFourState()) { return false; }
        lsbConstp->v3error(
            "Selection index is constantly unknown or tristated: " << lsbConstp->name());
    ...
    void warnSelConst(AstArraySel* nodep, int32_t fromLsb, int32_t fromMsb) {
        ...
        if (!bitConstp) { return; }
        if (warnSelTriAndReplace(nodep, bitConstp)) { return; }
        int32_t bit = bitConstp->toSInt();
        warnSelConstOOB(bit, bit, fromLsb, fromMsb, nodep);
    }
    // Produces a warning if constant select is outside of declared msb and lsb
    // Returns true in case of a fatal error - tristated LSB. Allowing tristated LSB would crash
    // later.
    bool warnSelConst(AstSel* nodep) {
        AstConst* lsbConstp = VN_CAST(nodep->lsbp(), Const);
        if (!lsbConstp) { return false; }
        if (warnSelTriAndReplace(nodep, lsbConstp)) { return true; }
        // Indexing unranged objects warning already handled in V3WidthSel
        if (!nodep->declRange().ranged()) { return false; }
    ...
    void visit(AstSel* nodep) override {
        ...
        if (m_vup->prelim()) {
            ...
            // We do not wanna advance further with a tristated LSB, as that would cause other
            // problems
            if (warnSelConst(nodep)) { return; }
    ...
    void visit(AstArraySel* nodep) override {
        ...
        assertAtExpr(nodep);
        if (m_vup->prelim()) {
            nodep->didWidth(true);  // Both passes done here
