...
class ConstraintExprVisitor final : public VNVisitor {
    ...
    void renameArrayExpr(AstSFormatF* const newp) const {
        if (m_structSel) {
            newp->name("%s.%s");
            // "%0x" (unpadded) to match the C-side "%x" used when declaring the
            // per-element solver variables; the index expression's width must
            // not add leading zeroes to the name.
